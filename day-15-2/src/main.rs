...
fn read_input(path: &str) -> Vec<Vec<Node>> {
    let field = read_input_small(path);
    let mut large_field: Vec<Vec<Node>> = Vec::new();
    ...
    for i in 0..5 {
        for row in &field {
            large_field.push(
                row.iter()
                    .map(|&x| (x - 1 + i) % 9 + 1)
                    .chain(row.iter().map(|&x| (x + i) % 9 + 1))
                    .chain(row.iter().map(|&x| (x + 1 + i) % 9 + 1))
                    .chain(row.iter().map(|&x| (x + 2 + i) % 9 + 1))
                    .chain(row.iter().map(|&x| (x + 3 + i) % 9 + 1))
                    .map(|x| Node {
                        risk: x,
                        finished: false,
                    })
...
fn read_input_small(path: &str) -> Vec<Vec<u32>> {
...
mod test {
    ...
    #[test]
    fn test_parse() {
        let _ = read_input("res/test-input.txt");
    }

    #[test]
    fn test_dijkstra() {
        assert_eq!(Some(315), compute_cheapest_path("res/test-input.txt"));
    }
}
